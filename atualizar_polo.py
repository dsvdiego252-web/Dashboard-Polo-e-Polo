...
atualizar_polo.py
=================
Lê a planilha Resumo_Farmacia_Polo_e_Polo.xlsx e atualiza os dados
...
# ── CONFIGURAÇÃO ──────────────────────────────────────────────────────────────
PLANILHA   = "Resumo_Farmacia_Polo_e_Polo.xlsx"   # planilha atualizada
...
COMMIT_MSG = "auto: atualiza dados Polo e Polo"
# ─────────────────────────────────────────────────────────────────────────────
...
_checar_datas(df_s, "Saidas")
_checar_datas(df_e, "Entradas")
...
_mes_map = {"01":"Jan","02":"Fev","03":"Mar","04":"Abr","05":"Mai","06":"Jun",
            "07":"Jul","08":"Ago","09":"Set","10":"Out","11":"Nov","12":"Dez"}
_labels = [_mes_map.get(_p.split("-")[1], _p) for _p in _periods]

_all = {k: round(sum(_by_period[_p].get(k, 0) for _p in _periods), 2)
        for k in ["vendas","compras","icms","icms_debito","icms_credito",
                  "pis_recolher","pis_debito","pis_credito",
                  "cofins_recolher","cofins_debito","cofins_credito"]}
...
RESUMO_OUTPUT = DADOS_DIR / "resumo_dados.js"
_resumo_js = "// Resumo inicial leve - gerado automaticamente pelo atualizar_polo.py\n"
_resumo_js += "var RESUMO_DATA=" + _json.dumps(_resumo, ensure_ascii=False, separators=(",",":")) + ";\n"
RESUMO_OUTPUT.write_text(_resumo_js, encoding="utf-8")
print(f"💾 Resumo salvo em: {RESUMO_OUTPUT}")
...
if GIT_PUSH:
    try:
        ...
        # Incluir resumo_dados.js (arquivo leve gerado automaticamente)
